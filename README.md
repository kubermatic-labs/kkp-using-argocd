...
**Note:** The sample code provided here to create Kubernetes clusters uses single VM control-plane. This is NOT recommended in any way as production. Always use HA control-plane for any production grade Kubernetes installation. Also please do NOT copy paste the values.yaml contents, espl the resource requests and limits. Those have been substantially reduced for demo installation. Production deployments can face resources crunch if the same resource allocations from the demo code are used.
